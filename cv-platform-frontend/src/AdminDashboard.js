...
function AdminDashboard() {
    ...
    const fetchCurriculums = async () => {
        ...
        try {
            // A rota para buscar todos os currículos para o admin continua a mesma
    ...
    const handleSelectForCompany = async (id) => {
        ...
        try {
            // Usamos a rota que você adicionou no server.js
            // NOTA: Ajustei a rota para ser consistente com as outras rotas de admin.
            // Veja a sugestão de melhoria no final da resposta.
            const response = await axios.put(`http://localhost:3001/api/admin/curriculos/${id}/select`, null, {
                headers: {
                    Authorization: `Bearer ${adminToken}`
                }
            });
    ...
    const copyLandingPageLink = () => {
        ...
        const link = `http://localhost:3000/empresa?ids=${ids}`;
